...
from sanhuo.models import weixin
from django.shortcuts import render
...
class WXBot:
    ...
    def proc_msg(self):
        ...
        if not weixin.objects.filter(uid=0):
            weixin.objects.create(name='me',flag=0,uid=0)
            td=weixin.objects.filter(uid=0)
    ...
    def run(self):
        ...
        for i in self.contact_list:
            k=k+1
            if not weixin.objects.filter(name=i['NickName'],uid=k):
                weixin.objects.create(name=i['NickName'],uid=k)
        self.proc_msg()
    ...
    def get_voice(self, msgid):
        ...
        with open(fn, 'wb') as f:
            f.write(data)
        return fn
class MyWXBot(WXBot):  #继承
    def handle_msg_all(self, msg):
        if msg['msg_type_id'] == 4 and msg['content']['type'] == 0:
            if not weixin.objects.filter(name=msg['user']['name']):
                weixin.objects.create(name=msg['user']['name'],flag=0)
            weixin.objects.filter(name=msg['user']['name']).update(contenthe=msg['content']['data'])
            f=open('sanhuo/templates/2.html','a')
            f.writelines(msg['user']['name'])
            f.writelines(" 的消息：")
            f.writelines(msg['content']['data'])
            f.writelines("<br>\n")
            f.close()
    def schedule(self):
        if weixin.objects.filter(flag=1).values():
            a=weixin.objects.filter(flag=1).values()
            f=open('sanhuo/templates/2.html','a')
            for i in a:
                f.writelines("回复:")
                f.writelines(i['name'])
                f.writelines(i['contentme'])
                f.writelines("&nbsp\n")
                f.writelines("<br>\n")
                self.send_msg(i['name'], i['contentme'])
            f.close()
            weixin.objects.filter(flag=1).update(flag=0)
  #      self.send_msg(u'5uw1st', u'test')




def index(request):
    denglu=weixin.objects.filter(uid=0,name='me')
    #denglu.delete()
    if threading.active_count()<=5 or not denglu:  #登陆
        denglu.delete()
        print threading.active_count()
        if threading.active_count()<=5:
            weixin.objects.filter(flag=0).delete()
            string='''<html>
                <script>
   ...
   window.setInterval("timedGetText()",3000);
    </script>
</html>'''
            f=open('sanhuo/templates/2.html','w')     #初始化显示页面
            f.write(string)
            f.close()
            t1=threading.Thread(target=test, args=(request,))
            t1.setDaemon(True)
            t1.start()
            time.sleep(5)

        return render(request,'alphago/home.html')
    else:                           #已登录
        #p = weixin.objects.get(uid=0,name='me')
        #p.delete()
        print threading.active_count()
        lists=[]
        listdict=weixin.objects.filter().exclude(name='me').values()
        for i in listdict:
            for key in i.keys():
                if key=='name':
                    lists.append(i[key])
        return render(request,'1.html',{'listdict':lists})
def add(request):            #发送消息
    a = request.POST['userid']
    b=request.POST['chat']
    weixin.objects.filter(name=a).update(contentme=b,flag=1)
    lists=[]
    listdict=weixin.objects.filter().exclude(name='me').values()
    for i in listdict:
        for key in i.keys():
...
def flash(request):
    if weixin.objects.filter(uid=0):
        print 'hello world!----------------------------------------------------'
        lists=[]
        listdict=weixin.objects.all().values()
        for i in listdict:
            for key in i.keys():
                if key=='name':
                    lists.append(i[key])
